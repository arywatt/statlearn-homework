...
idx.tr = createDataPartition(y = data$genre, p = .75, list = FALSE)
head(idx.tr)
...
rf = randomForest(genre ~ ., data = data.scaled, 
                  na.action = na.omit,
                  importance = FALSE)
...
(phi <- dd.eigen$vectors[,1:2])

phi <- -phi
row.names(phi) <- colnames(data.scaled.num)
