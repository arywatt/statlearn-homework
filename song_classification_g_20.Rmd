...
library(audiolyzR)
library(reticulate)
use_python("C:/Users/watt/Anaconda3/python.exe", required = T)

knitr::opts_chunk$set(engine.path = list(
  python = "C:/Users/watt/Anaconda3/python.exe"
))
...
getSpecgram <- function (x, winsize, fs ,overlap){
  sp  <- specgram(as.numeric(x), n = winsize, Fs = fs,  overlap = overlap)
  return(sp)
...
getSpec <- function (x, winsize, fs ){
  sp  <- meanspec(x,  fs = fs, wl = winsize, fftw = FALSE,norm  =TRUE)
  return(sp)
}
...
for( j in 1:N){
  ...
  print('------------------ Specprop---------------------------------------------')
  # 
  # spec = getSpec(xWave,winsize = winsize , rate)
  # specprops = GetSpecProps(spec,rate)
  # str(specprops)
  # 
  # 
  # for (i in 1:length(specprops)){
  #   features = append(features,specprops[[i]])
  # }
  # features_names = append(features_names,names(specprops))
   ...
   # features_names = append(features_names,'zcr')
...
#str(featurelist)
# 
# data = as.data.frame(matrix(unlist(featurelist), nrow=length(unlist(featurelist[2]))))
# 
# length(features_names)
# row.names(data) = features_names
# 
# data2 = t(data)
...
We extract other features with python
